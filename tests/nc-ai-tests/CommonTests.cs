...
public abstract class CommonTests
{
	public CommonTests()
	{
		...
		ContextCache = services.GetRequiredService<IAiContextCache>();
	}

	public required IChatClient Client { get; set; }
	public IAiContextCache ContextCache { get; }
	...
	[Fact]
	public async Task CachesContext()
	{
		var content = new TextContent("Your name is Tiberius. You are a poet. Answer all questions using iambic pentameter.");
		await ContextCache.SetContextAsync("starpoet", content);
		var instructions = await ContextCache.GetContextAsync("starpoet");

		var agent = new ContextCacheClient(Client, ContextCache);
		var question = new TextContent("What is your name? What is your profession?");
		var userMessage = new ChatMessage(ChatRole.User, [instructions, question]);
		var response = await agent.GetResponseAsync([userMessage], new ChatOptions { });
	}
