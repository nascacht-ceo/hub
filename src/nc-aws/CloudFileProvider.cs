...
public class CloudFileProvider : ICloudFileProvider
{
    ...
    public CloudFileProvider(IAmazonS3 s3Client, string bucketName, ILogger? logger = null)
    {
        S3Client = s3Client;
    ...
    public async Task<ICloudFileInfo> GetFileInfoAsync(string filePath, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return new CloudFileInfo(this, filePath, metadataResponse);
        }
        catch (AmazonS3Exception ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
        {
            ...
            {
                return new CloudFileInfo(this, listResponse.S3Objects.First());
            }
            else
            {
                return new CloudFileInfo(this, filePath);
            }
        }
    }
    ...
    private async IAsyncEnumerable<ICloudFileInfo> GetS3ContentsAsync(string directoryPath, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        do
        {
            ...
            {
                yield return new CloudFileInfo(this, prefix);
            }

            // Yield files
            foreach (var s3Object in listResponse.S3Objects)
            {
                yield return new CloudFileInfo(this, s3Object);
            }
